...
const InteractiveMonkey: React.FC<MonkeyProps> = ({ isTyping, isPasswordVisible }) => {
  ...
  const calculateEyeDirection = () => {
    const monkeyRect = { left: window.innerWidth * 0.7, top: window.innerHeight * 0.4 };
  ...
  return (
    <div className="relative w-80 h-80 flex items-center justify-center">
      ...
      <div className="relative w-64 h-64 bg-gradient-to-br from-orange-200 to-orange-300 rounded-full shadow-2xl transform transition-all duration-300 hover:scale-105">
        ...
        {/* Ears */}
        <div className="absolute -top-8 left-8 w-16 h-16 bg-orange-300 rounded-full"></div>
        ...
        <div className="absolute top-16 left-12 w-12 h-12 bg-white rounded-full shadow-inner">
          {eyesOpen && !isPasswordVisible ? (
            <div 
              className="w-6 h-6 bg-black rounded-full transition-all duration-100 ease-out"
              style={{
                transform: `translate(${6 + eyeDirection.x}px, ${6 + eyeDirection.y}px)`
              }}
            >
            ...
            <div className="w-full h-2 bg-black mt-5 rounded-full"></div>
          )}
        </div>
        ...
        {/* Nose */}
        <div className="absolute top-28 left-1/2 transform -translate-x-1/2 w-4 h-3 bg-orange-400 rounded-full"></div>
        
        {/* Mouth */}
        <div className={`absolute top-36 left-1/2 transform -translate-x-1/2 transition-all duration-300 ${
          isTyping ? 'w-8 h-4 bg-orange-400 rounded-full' : 'w-6 h-2 bg-orange-400 rounded-full'
        }`}>
          {isTyping && (
        ...
        {/* Cheeks - blush when typing */}
        {isTyping && (
          <>
            <div className="absolute top-24 left-4 w-6 h-6 bg-pink-300 rounded-full opacity-60 animate-pulse"></div>
            <div className="absolute top-24 right-4 w-6 h-6 bg-pink-300 rounded-full opacity-60 animate-pulse"></div>
          </>
        )}
      </div>
